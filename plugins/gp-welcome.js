...
async function getUserProfilePic(conn, jid) {
    ...
    if (!buffer) { if (!defaultAvatarBuffer) await preloadDefaultAvatar(); buffer = defaultAvatarBuffer; }
...
async function getGroupBackgroundImage(groupJid, conn) {
    ...
    if (!buffer) {
        try {
            ...
            await fs.access(fallback); buffer = await fs.readFile(fallback);
...
const WelcomeCard = ({ backgroundUrl, pfpUrl, isGoodbye, username, groupName }) => {
    ...
    const styles = `
        ...
        .card{background:rgba(0,0,0,0.6);padding:60px;border-radius:40px;text-align:center;color:white;width:85%;}
        img{width:260px;height:260px;border-radius:50%;border:8px solid white;object-fit:cover;margin-bottom:30px;}
